 ...
 *
 * 「在线彩色、离线灰色」的实现与它的两个前提(为什么是 grayscale 而不是覆盖 fill、
 * 为什么苹果必须是彩虹版)记在本文件底部 `<style scoped>` 里那条规则旁边 —— 
 * 那里是会被改动的人先看到的地方。
 */
...
const props = defineProps<{
  ...
  hint?: string;
  /** 边长(px)。默认 14 适配顶栏 chip,设备卡头像位传 18。
   *  为什么是 prop 而不是外层写 `.avatar .os-icon svg { width:18px }` 覆盖:那条选择器
   *  横跨两个组件(宿主 `.avatar` 属于 DeviceList),样式收进本组件后 scoped 选不到它。
   *  尺寸本来就是调用方决定的事,显式传参比隔层覆盖更诚实。 */
  size?: number;
}>();

const kind = computed(() => osIconKind(props.platform));
const label = computed(() => props.hint ?? osIconLabel(props.platform));
const box = computed<number>(() => props.size ?? 14);
...
<template>
  <span class="os-icon" :class="{ 'is-offline': offline }" :title="label" :aria-label="label" role="img">
    ...
    <svg v-if="kind === 'windows'" viewBox="0 0 16 16" :width="box" :height="box" aria-hidden="true">
    ...
    <svg v-else-if="kind === 'macos'" viewBox="0 0 24 24" :width="box" :height="box" aria-hidden="true">
      <defs>
    ...
    <svg v-else-if="kind === 'linux'" viewBox="0 0 16 16" :width="box" :height="box" aria-hidden="true">
      <!-- 墨色:头、身、两鳍 -->
    ...
    <svg v-else viewBox="0 0 16 16" :width="box" :height="box" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" aria-hidden="true">
...
/* 不在线 = 去色 + 减淡。
   为什么走 filter 而不是「一条规则把子元素 fill 刷成 var(--muted)」:后者字更少,也确实施得通
   (SVG 的 fill 是 presentation attribute,优先级低于任何 CSS 规则),但它会把 Tux 的肚皮和
   身子刷成同一个灰 —— 图标塌成一块没有轮廓的灰斑,形状信息全丢。grayscale 保留亮度对比:
   四格窗去饱和后仍是四块不同明度的灰,企鹅肚子仍比身子亮。
   这条实现方式有个硬前提:苹果必须是彩虹版(见上方模板)。单色黑剪影去饱和后与不去饱和
   完全相同,在线 / 离线会长得一模一样。 */
.os-icon.is-offline {
  filter: grayscale(1);
  opacity: 0.5;
